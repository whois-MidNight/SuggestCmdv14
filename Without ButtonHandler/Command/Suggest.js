...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        {name: "<:info:1029480432821075988> | New Suggestion",value: `${name} ` + `:\n \`${description}\``,inline: false},
    ...
    interaction.reply({content: `:white_check_mark: | Your suggestion has been succesfully submitted in our [server](https://discord.com/channels/1020560054128226346/1025747095216455770).`,ephemeral: true});
    try {
      const msg = await client.channels.cache.get("1029738854892195860")
        .send({ embeds: [embed], components: [row] });
      await SuggestDB.create({
        Guild: guild.id,
        Message: msg.id,
        Details: [{MemberID: interaction.user.id,Type: type,Suggestion: name}],
      });
    } catch (error) {
      console.log(error);
    }
